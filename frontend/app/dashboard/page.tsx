...
export default function DashboardPage() {
  ...
  const loadDocumentsData = async (): Promise<Document[]> => {
    try {
      ...
      params.append('limit', '1000') // Get more for filtering
  ...
  const loadDocuments = async () => {
    try {
      ...
      let filtered = allDocs.filter(doc => {
        ...
        // Keyword search (already handled by API, but can refine)
        if (filters.keywords.length > 0) {
          const docText = `${doc.filename} ${doc.document_type}`.toLowerCase()
                ...
                <div className="space-y-3">
                  {documents.map((doc) => (
                    <DocumentCard
                      ...
                      document={doc}
                      onClick={() => {
                        // Navigate to document view if available
              ...
              {documents.length > 0 && (
                <div className="mt-6 flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    Showing {pagination.offset + 1} to {Math.min(pagination.offset + pagination.limit, filteredCount)} of {filteredCount}
                  </div>
                  <div className="flex gap-2">
                    <button
                    ...
                    <button
                      ...
                      disabled={pagination.offset + pagination.limit >= filteredCount}
                      className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next
                    </button>
...
function FilterSection({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      {isExpanded && <div className="px-4 pb-4 pt-2 border-t border-gray-100">{children}</div>}
    </div>
  )
}
